...
class MovieTheater(Theater):
    ...
    def add_movie(cls):
        cls.total_movies += 1
        return '영화 추가를 성공했습니다.'
    
    @staticmethod
    def desciption(a):
        print(f'영화관 이름: {a.name}')
        print(f'총 좌석 수: {a.total_seats}')
        print(f'예약된 좌석 수: {a.reserved_seats}')
        print(f'총 영화 수: {a.total_movies}')

mv = MovieTheater('메가박스',100,0)
MovieTheater.add_movie()

mv.reserve_seat()
mv.reserve_seat()

MovieTheater.desciption(mv)
